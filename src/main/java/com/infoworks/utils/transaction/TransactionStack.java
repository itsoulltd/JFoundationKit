...
public class TransactionStack implements TaskStack, StackedTaskStateListener {
    ...
    @Override
    public void before(Task task, StackManager.State state) {
        synchronized (this){
            if (state == StackManager.State.Forward){
                Task popped = beanStack.pop();
                passedStack.push(popped);
            }else if (state == StackManager.State.Backward){
    ...
    @Override
    public void after(Task task, StackManager.State state) {
        synchronized (this){
            if (state == StackManager.State.Forward){
                //TODO:
            }else if (state == StackManager.State.Backward){
