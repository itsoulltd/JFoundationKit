...
public class LettuceDataSource implements iMemorySource {
    ...
    public Map<String, Object> read(String key) {
        ...
        if (MessageParser.isValidJson(json)){
            try {
                Map<String, Object> data = MessageParser.unmarshal(new TypeReference<Map<String, Object>>() {}, json);
                return data;
            } catch (IOException e) {}
        }
        return null;
    ...
    public void put(String key, Map<String, Object> entity, long ttl) {
        RedisCommands<String, String> cmd = connection.sync();
        try {
            if (ttl > 0l) cmd.set(key, MessageParser.marshal(entity), SetArgs.Builder.ex(ttl));
            else cmd.set(key, MessageParser.marshal(entity));
        } catch (IOException e) {}
    }
